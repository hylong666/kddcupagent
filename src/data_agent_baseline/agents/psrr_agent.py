...
class PlanSolveReActReflexionAgent:
    """Plan-Solve + ReAct agent wrapper.

    - Planning is done once at the start (Plan-Solve).
    - Execution uses the existing `ReActAgent` (no changes to react.py).
    - Reflexion is handled outside the agent (runner) so outputs can be stored.
    """
    ...
    def __init__(
        ...
        *,
        ...
        tools: ToolRegistry,
        project_root: Path,
        config: PSRRAgentConfig | None = None,
        step_callback=None,
    ) -> None:
        self.model = model
        self.tools = tools
        self.project_root = project_root
        self.config = config or PSRRAgentConfig()
    ...
    def _build_psrr_system_prompt(
        ...
        return (PSRR_REACT_SYSTEM_PROMPT.strip() + plan_block + risk_block).strip()
    ...
    def run(self, task: PublicTask, *, improvement_hints: list[str] | None = None) -> AgentRunResult:
        ...
        try:
            ...
            plan = PlanSolveResult(
                plan=[
                    ...
                    "Inspect relevant docs/JSON/CSV previews to locate the needed data.",
                    "If a sqlite/db exists, inspect schema then run focused read-only SQL.",
                    "Use execute_python for lightweight parsing/aggregation if needed.",
        ...
        react_agent = ReActAgent(
            model=self.model,
            tools=self.tools,
            config=ReActAgentConfig(max_steps=self.config.max_steps),
